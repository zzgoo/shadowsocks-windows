...
namespace Shadowsocks.Protocol
{
    public class TcpPipeListener
    {
        ...
        public TcpPipeListener(IPEndPoint endPoint, IEnumerable<IStreamService> services) : base()
        {
        ...
        public async Task Start()
        {
            ...
            while (true)
            {
                ...
                foreach (var svc in _services)
                {
                    ...
                    {
                        // todo: save to list, so we can optinally close them
        ...
        private async Task RunService(IStreamService svc, SocketConnection conn)
        {
            ...
            var s5c = new Socks5Client();
            var tpc = new TcpPipeClient();
            var t2 = tpc.Connect(IPEndPoint.Parse("127.0.0.1:1081"), s5tcp.DownSide, null);
